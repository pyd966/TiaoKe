...
4. 立即休息：从工作中或提醒状态通过托盘菜单立即进入休息倒计时，并结束当前工作计时。
5. 暂停休息提醒：支持暂停 15 分钟、30 分钟、1 小时、2 小时和“今天不再提醒”；暂停到期后开始完整的新一轮工作计时。
6. 系统托盘：显示当前状态与剩余时间，提供立即休息、重置本轮、暂停休息提醒、设置、退出。
7. 开机自启动：默认在首次设置中询问，之后可随时开关；仅修改当前用户配置，不需要管理员权限。
8. 设置持久化：工作时长、休息时长、提醒位置、开机启动和外观设置保存在本机。
9. 单实例：重复启动时唤起已有实例的设置页，不产生多个计时器。
...
stateDiagram-v2
    ...
    Working --> Resting: 用户选择立即休息
    ReminderDue --> Resting: 用户选择立即休息
...
- `立即休息`：无论当前处于 `Working` 还是 `ReminderDue`，都直接进入 `Resting`；当前工作轮次不再恢复。
- `暂停休息提醒`：对 `Working` 或 `ReminderDue` 生效；暂停期间不显示休息提醒，到期后从完整工作时长开始。
...
1. 状态行（禁用项）：`距下次远眺 12:34` / `等待开始远眺` / `休息中 00:13` / `已暂停至 16:30`
2. `立即休息`
3. `重置本轮计时`
4. `暂停休息提醒` 子菜单：15 分钟、30 分钟、1 小时、2 小时、今天不再提醒
5. 分隔线
6. `设置…`
7. `退出眺刻`
...
- 实现工作、提醒、休息和暂停提醒状态，覆盖重置、立即休息与休眠恢复。
- 使用可替换时钟编写单元测试。
